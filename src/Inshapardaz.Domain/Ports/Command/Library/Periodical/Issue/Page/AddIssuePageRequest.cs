...
public class AddIssuePageRequestHandler(
    IIssueRepository issueRepository,
    IIssuePageRepository issuePageRepository,
    IAmACommandProcessor commandProcessor)
    : RequestHandlerAsync<AddIssuePageRequest>
{
    ...
    public override async Task<AddIssuePageRequest> HandleAsync(AddIssuePageRequest command, CancellationToken cancellationToken = new CancellationToken())
    {
        ...
        if (existingIssuePage == null)
        {
            ...
            command.Result = await issuePageRepository.AddPage(command.LibraryId, command.IssuePage, cancellationToken);
            command.IsAdded = true;
        }
        else
        {
            command.Result = await issuePageRepository.UpdatePage(command.LibraryId, command.IssuePage, cancellationToken);
        }
