...
namespace Inshapardaz.Api.Ports
{
    ...
    public class PostRelationshipRequestHandler : RequestHandlerAsync<PostRelationshipRequest>
    {
        ...
        public override async Task<PostRelationshipRequest> HandleAsync(PostRelationshipRequest command, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            var sourceWord = await _queryProcessor.ExecuteAsync(new WordByIdQuery { WordId = command.WordId });
            if (sourceWord == null)
            {
                throw new NotFoundException();
            }

            var relatedWord = await _queryProcessor.ExecuteAsync(new WordByIdQuery { WordId = command.Relationship.RelatedWordId });
            if (relatedWord == null)
            {
                throw new NotFoundException();
            }

            var dictionary2 = await _queryProcessor.ExecuteAsync(new DictionaryByWordIdQuery { WordId = command.Relationship.RelatedWordId });
            ...
            await _commandProcessor.SendAsync(addCommand);

            var newRelationship = await _queryProcessor.ExecuteAsync(new RelationshipByIdQuery { Id = addCommand.RelationId });
