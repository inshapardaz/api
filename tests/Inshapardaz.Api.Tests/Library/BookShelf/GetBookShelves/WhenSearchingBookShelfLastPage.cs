...
{
    [TestFixture]
    public class WhenSearchingBookShelfLastPage() : TestBase(Domain.Models.Role.Reader)
    {
        ...
        public async Task Setup()
        {
            ...
            _response = await Client.GetAsync($"/libraries/{LibraryId}/bookshelves?query={searchTerm}&pageNumber=5&pageSize=10");
        ...
        [Test]
        public void ShouldHavePreviousLinks() => _assert.ShouldHavePreviousLink($"/libraries/{LibraryId}/bookshelves", 4, 10, new KeyValuePair<string, string>("query", searchTerm));

        [Test]
        public void ShouldReturnExpectedSeries()
        {
            var expectedItems = BookShelfBuilder.BookShelves.Where(a => a.Name.Contains(searchTerm)).OrderBy(a => a.Name).Skip(4 * 10).Take(10);
