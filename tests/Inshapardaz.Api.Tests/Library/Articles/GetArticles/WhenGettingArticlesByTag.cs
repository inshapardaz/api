...
namespace Inshapardaz.Api.Tests.Library.Articles.GetArticles
{
    ...
    public class WhenGettingArticlesByTag : TestBase
    {
        ...
        public async Task Setup()
        {
            ...
            ArticleBuilder.WithLibrary(LibraryId).WithTag(_tag1).Build(30);
            ArticleBuilder.WithLibrary(LibraryId).WithTag(_tag2).Build(10);

            _response = await Client.GetAsync($"/libraries/{LibraryId}/articles?pageNumber=2&pageSize=10&tagId={_tag2.Id}");
        ...
        {
            var allTagArticles = ArticleBuilder.ArticleTags.Where(x => x.Value.Contains(_tag2.Id)).Select(x => x.Key).ToList();
