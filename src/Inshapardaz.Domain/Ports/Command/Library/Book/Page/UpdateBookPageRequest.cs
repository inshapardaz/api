...
public class UpdateBookPageRequestHandler(
    IBookRepository bookRepository,
    IBookPageRepository bookPageRepository,
    IAmACommandProcessor commandProcessor)
    : RequestHandlerAsync<UpdateBookPageRequest>
{
    ...
    {
        var book = await bookRepository.GetBookById(command.LibraryId, command.BookPage.BookId, command.AccountId, cancellationToken);
        if (book == null)
        {
            throw new BadRequestException();
        }

        var existingBookPage = await bookPageRepository.GetPageBySequenceNumber(command.LibraryId, command.BookPage.BookId, command.BookPage.SequenceNumber, cancellationToken);
        ...
        if (existingBookPage == null)
        {
            ...
            command.Result.BookPage = await bookPageRepository.AddPage(command.LibraryId, command.BookPage, cancellationToken);
            command.Result.BookPage.Text = command.BookPage.Text;
            command.Result.HasAddedNew = true;
        }
        else
        {
            command.Result.BookPage = await bookPageRepository.UpdatePage(command.LibraryId, command.BookPage.BookId, command.BookPage.SequenceNumber, saveContentCommand.Result.Id, existingBookPage.ImageId ?? 0, command.BookPage.Status, command.BookPage.ChapterId, cancellationToken);
            command.Result.BookPage.Text = command.BookPage.Text;
        }

        var previousPage = await bookPageRepository.GetPageBySequenceNumber(command.LibraryId, command.BookPage.BookId, command.SequenceNumber - 1, cancellationToken);
        var nextPage = await bookPageRepository.GetPageBySequenceNumber(command.LibraryId, command.BookPage.BookId, command.SequenceNumber + 1, cancellationToken);
