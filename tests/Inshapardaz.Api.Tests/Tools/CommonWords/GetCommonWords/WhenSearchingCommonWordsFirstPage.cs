...
{
    [TestFixture]
    public class WhenSearchingCommonWordsFirstPage() : TestBase(Domain.Models.Role.Reader)
    {
        ...
        [Test]
        public void ShouldHaveSelfLink() => _assert.ShouldHaveSelfLink($"/tools/{_language}/words", new KeyValuePair<string, string>("query", _searchedWord));

        [Test]
        public void ShouldNotHaveCreateLink() => _assert.ShouldNotHaveCreateLink();

        [Test]
        public void ShouldHaveNextLink() => _assert.ShouldHaveNextLink($"/tools/{_language}/words", 2, 10, new KeyValuePair<string, string>("query", _searchedWord));
