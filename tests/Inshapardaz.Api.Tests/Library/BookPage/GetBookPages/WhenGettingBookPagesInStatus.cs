...
namespace Inshapardaz.Api.Tests.Library.BookPage.GetBookPages
{
    ...
    public class WhenGettingBookPagesInStatus(EditingStatus status) : TestBase(Role.Reader)
    {
        ...
        public async Task Setup()
        {
            ...
            _book = BookBuilder.WithLibrary(LibraryId).WithPages(20).WithStatus(status, 15).Build();

            _response = await Client.GetAsync($"/libraries/{LibraryId}/books/{_book.Id}/pages?pageSize=10&pageNumber=1&status={status.ToDescription()}");
        ...
        [Test]
        public void ShouldHaveNextLink() => _assert.ShouldHaveNextLink($"/libraries/{LibraryId}/books/{_book.Id}/pages", 2);
        ...
        {
            var expectedItems = BookBuilder.GetPages(_book.Id).Where(p => p.Status == status).OrderBy(p => p.SequenceNumber).Take(10);
