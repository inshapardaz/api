...
{
    [TestFixture]
    public class WhenGettingMeaningsForContext : IntegrationTestBase
    {
        ...
        private Domain.Entities.Meaning _meaning2;
        private Domain.Entities.Meaning _meaning3;
        ...
        public async Task Setup()
        {
            ...
            _meaning2 = MeaningDataHelper.CreateMeaning(_dictionary.Id, _word.Id, new Domain.Entities.Meaning { Context = "ctx2", Value = "sdsd2", Example = "None" });
            _meaning3 = MeaningDataHelper.CreateMeaning(_dictionary.Id, _word2.Id, new Domain.Entities.Meaning { Context = "ctx1", Value = "erer2", Example = "None" });

            Response = await GetContributorClient(_userId).GetAsync($"/api/dictionaries/{_dictionary.Id}/words/{_word.Id}/meanings/contexts/ctx1");
        ...
        [Test]
        public void ShouldNotContainMeaningForTheWordWithDifferentContext()
        {
            _view.ShouldNotContain(m => m.Id == _meaning2.Id);
        }
