...
namespace Inshapardaz.Api.Tests.Library.Periodical.IssuePage.GetIssuePages
{
    ...
    public class WhenGettingIssuePagesWithImagesAsWithPermission : TestBase
    {
        private IssueDto _issue;
        private HttpResponseMessage _response;
        private PagingAssert<IssuePageView> _assert;

        public WhenGettingIssuePagesWithImagesAsWithPermission(Role role)
            : base(role)
        {
        }
        ...
        {
            _issue = IssueBuilder.WithLibrary(LibraryId).WithPages(9, true).Build();
