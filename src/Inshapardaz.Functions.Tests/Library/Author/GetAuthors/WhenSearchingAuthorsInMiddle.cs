...
{
    [TestFixture]
    public class WhenSearchingAuthorsInMiddle : LibraryTest<Functions.Library.Authors.GetAuthors>
    {
        ...
        {
            _assert.ShouldHaveNextLink($"/api/library/{LibraryId}/authors", 4, 10, "query", "SearchAuthor");
        }

        [Test]
        public void ShouldHavePreviousLinks()
        {
            _assert.ShouldHavePreviousLink($"/api/library/{LibraryId}/authors", 2, 10, "query", "SearchAuthor");
        }

        [Test]
        public void ShouldReturnExpectedAuthors()
        {
            var expectedItems = _builder.Authors.Where(a => a.Name.Contains("SearchAuthor")).OrderBy(a => a.Name).Skip(2 * 10).Take(10);
