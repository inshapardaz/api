...
{
    [TestFixture]
    public class WhenGettingMeaningsForContextWithNoAccess : IntegrationTestBase
    {
        ...
        public async Task Setup()
        {
            ...
            var meaning1 = MeaningDataHelper.CreateMeaning(_dictionary.Id, _word.Id, new Domain.Entities.Dictionary.Meaning { Context = "ctx1", Value = "sdsd1", Example = "None" });
            var meaning2 = MeaningDataHelper.CreateMeaning(_dictionary.Id, _word.Id, new Domain.Entities.Dictionary.Meaning { Context = "ctx2", Value = "sdsd2", Example = "None" });

            Response = await GetContributorClient(Guid.NewGuid()).GetAsync($"/api/dictionaries/{_dictionary.Id}/words/{_word.Id}/meanings/contexts/ctx1");
