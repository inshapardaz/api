...
namespace Inshapardaz.Ports.Database.Repositories
{
    public class TranslationRepository : ITranslationRepository
    {
        ...
        public async Task<Translation> AddTranslation(int dictionaryId, long wordId, Translation translation, CancellationToken cancellationToken)
        {
            ...
            var item = translation.Map<Translation, Entities.Translation>();
            word.Translation.Add(item);

            await _databaseContext.SaveChangesAsync(cancellationToken);

            return item.Map<Entities.Translation, Translation>();
        }
        ...
        public async Task<Translation> GetTranslationById(int dictionaryId, long wordId, long translationId, CancellationToken cancellationToken)
        {
            var translation = await _databaseContext.Translation
                                         .SingleOrDefaultAsync(t => t.Word.DictionaryId == dictionaryId &&
                                                                    ...
                                                                    t.Id == translationId,
                                                               cancellationToken);
            return translation.Map<Entities.Translation, Translation>();
        }
        ...
        public async Task<IEnumerable<Translation>> GetWordTranslationsByLanguage(int dictionaryId, long wordId, Languages language, CancellationToken cancellationToken)
        {
            return await _databaseContext.Translation
                                         .Where(t => t.Word.DictionaryId == dictionaryId &&
                                                     t.WordId == wordId &&
                                                     t.Language == language)
                                         .Select(t => t.Map<Entities.Translation, Translation>())
