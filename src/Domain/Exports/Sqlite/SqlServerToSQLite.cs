    ...
    public class SqlServerToSQLite
    {
            ...
            bool createTriggers, bool createViews)
        {
            ...
            WaitCallback wc = new WaitCallback(delegate(object state)
            {
                ...
                {
                    //_logger.LogError("Failed to convert SQL Server database to SQLite database", ex);
                    _isActive = false;
            ...
            bool createTriggers, bool createViews)
        {
            ...
            if (createTriggers)
                AddTriggersForForeignKeys(sqlitePath, ds.Tables, handler);
        ...
        /// <param name="handler">A handler to handle progress notifications.</param>
        private static void CopySqlServerRowsToSQLiteDB(string sqlConnString, string sqlitePath, List<TableSchema> schema, SqlConversionHandler handler)
        {
            CheckCancelled();
            handler(false, true, 0, "Preparing to insert tables...");
            //_logger.LogDebug("preparing to insert tables ...");
                ...
                using (SqliteConnection sqconn = new SqliteConnection(sqliteConnString))
                {
                    ...
                    for (int i = 0; i < schema.Count; i++)
                    {
                        ...
                        try
                        {
                            ...
                            using (SqlDataReader reader = query.ExecuteReader())
                            {
                                ...
                                while (reader.Read())
                                {
                                    ...
                                    for (int j = 0; j < schema[i].Columns.Count; j++)
                                    {
                                        ...
                                        insert.Parameters[pname].IsNullable = schema[i].Columns[j].IsNullable;
                                        pnames.Add(pname);
                                    }
                            ...
                            //_logger.LogDebug("finished inserting all rows for table [" + schema[i].TableName + "]");
                        }
                        catch (System.Exception ex)
                        {
        ...
        private static object CastValueForColumn(object val, ColumnSchema columnSchema)
        {
            ...
            switch (dt)
            {
                ...
                default:
                    //_logger.LogError("argument exception - illegal database type");
        ...
        private static SqliteCommand BuildSQLiteInsert(TableSchema ts)
        {
            ...
            for (int i = 0; i < ts.Columns.Count; i++)
            {
                ...
                SqliteParameter prm = new SqliteParameter(pname, dbType, ts.Columns[i].Length , ts.Columns[i].ColumnName);
        ...
        private static SqliteType GetDbTypeOfColumn(ColumnSchema cs)
        {
            ...
            //_logger.LogError("illegal db type found");
            throw new System.Exception("Illegal DB type found (" + cs.ColumnType + ")");
        }
        ...
        {
            //_logger.LogDebug("Creating SQLite database...");
            ...
            using (SqliteConnection conn = new SqliteConnection(sqliteConnString))
            {
                ...
                foreach (TableSchema dt in schema.Tables)
                {
                    ...
                    {
                        //_logger.LogError("AddSQLiteTable failed", ex);
                        throw;
                    }
                    count++;
                    CheckCancelled();
                    ...
                    //_logger.LogDebug("added schema for SQLite table [" + dt.TableName + "]");
                } // foreach
                ...
                if (createViews)
                {
                    foreach (ViewSchema vs in schema.Views)
                    {
                        ...
                        {
                            //_logger.LogError("AddSQLiteView failed", ex);
                            throw;
                        } // catch
                        count++;
                        CheckCancelled();
                        ...
                        //_logger.LogDebug("added schema for SQLite view [" + vs.ViewName + "]");

                    } // foreach
                } // if
            } // using

            //_logger.LogDebug("finished adding all table/view schemas for SQLite database");
        }
        ...
        private static string BuildColumnStatement(ColumnSchema col, TableSchema ts, ref bool pkey)
        {
            ...
            //_logger.LogDebug("DEFAULT VALUE BEFORE [" + col.DefaultValue + "] AFTER [" + defval + "]");
            if (defval != string.Empty && defval.ToUpper().Contains("GETDATE"))
            {
                //_logger.LogDebug("converted SQL Server GETDATE() to CURRENT_TIMESTAMP for column [" + col.ColumnName + "]");
            ...
            SqlTableSelectionHandler selectionHandler)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                for (int i = 0; i < tableNames.Count; i++)
                {
                    ...
                    //_logger.LogDebug("parsed table schema for [" + tname + "]");
                } // foreach
            } // using

            //_logger.LogDebug("finished parsing all tables in SQL Server schema");
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        //_logger.LogDebug("parsed view schema for [" + vs.ViewName + "]");
                    } // while
                } // using
        ...
        {
            if (_cancelled)
                throw new  System.Exception("User cancelled the conversion") ;
        }
        ...
        private static TableSchema CreateTableSchema(SqlConnection conn, string tableName, string tschma)
        {
            ...
            {
                //_logger.LogWarning("failed to read index information for table [" + tableName + "]");
            } // catch
        ...
        private static void ValidateDataType(string dataType)
        {
            if (dataType == "int" || dataType == "smallint" ||
                ...
                return;
            throw new InvalidDataException($"Validation failed for data type [{dataType}]");
        }
        ...
        private static IndexSchema BuildIndexSchema(string indexName, string desc, string keys)
        {
            ...
            foreach (string p in keysParts)
            {
                ...
                if (!m.Success)
                {
                    throw new InvalidDataException($"Illegal key name [{p}] in index [{indexName}]");
                }
        ...
        private static void AddTriggersForForeignKeys(string sqlitePath, IEnumerable<TableSchema> schema, SqlConversionHandler handler)
        {
            // Connect to the newly created database
            string sqliteConnString = CreateSQLiteConnectionString(sqlitePath);
            using (SqliteConnection conn = new SqliteConnection(sqliteConnString))
            {
                ...
                foreach (TableSchema dt in schema)
                {
                    ...
                    {
                        //_logger.LogError("AddTableTriggers failed", ex);
                        throw;
                    }
                }
            ...
            //_logger.LogDebug("finished adding triggers to schema");
        }
