...
public class SqlServerDatabaseFileStorage(SqlServerConnectionProvider connectionProvider) : IFileStorage
{
    ...
    public Task<string> StoreImage(string name, byte[] content, string mimeType, CancellationToken cancellationToken) => StoreFile(name, content, mimeType, cancellationToken);
