...
public class UpdateBookToBookShelfRequest(int libraryId, int bookShelfId, int bookId, int index)
...
public class UpdateBookToBookShelfRequestHandler(
    IBookShelfRepository bookShelfRepository,
    IBookRepository bookRepository,
    IUserHelper userHelper)
    : RequestHandlerAsync<UpdateBookToBookShelfRequest>
{
    [LibraryAuthorize(1)]
    public override async Task<UpdateBookToBookShelfRequest> HandleAsync(UpdateBookToBookShelfRequest command, CancellationToken cancellationToken = new CancellationToken())
    {
        ...
        var result = await bookShelfRepository.GetBookFromBookShelfById(command.LibraryId, command.BookShelfId, command.BookId, cancellationToken);
        ...
        else
        {
            ...
            await bookShelfRepository.UpdateBookToBookShelf(command.LibraryId, result, cancellationToken);
        }
