...
namespace Inshapardaz.Api.Tests.Library.Series.GetSeries
{
    ...
    public class WhenSearchingSeriesInMiddle : TestBase
    {
        ...
        public async Task Setup()
        {
            ...
            _response = await Client.GetAsync($"/library/{LibraryId}/series?query=SearchSeries&pageNumber=3&pageSize=10");
        ...
        {
            _assert.ShouldHaveNextLink($"/library/{LibraryId}/series", 4, 10, "query", "SearchSeries");
        }

        [Test]
        public void ShouldHavePreviousLinks()
        {
            _assert.ShouldHavePreviousLink($"/library/{LibraryId}/series", 2, 10, "query", "SearchSeries");
        }
