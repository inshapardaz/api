...
{
    [TestFixture]
    public class WhenSearchingCommonWordsInMiddle() : TestBase(Domain.Models.Role.Reader)
    {
        ...
        public async Task Setup()
        {
            ...
            _response = await Client.GetAsync($"/tools/{_language}/words?query={_searchedWord}&pageNumber=3&pageSize=10");
        ...
        [Test]
        public void ShouldHaveSelfLink() => _assert.ShouldHaveSelfLink($"/tools/{_language}/words", 3, 10, new KeyValuePair<string, string>("query", _searchedWord));

        [Test]
        public void ShouldNotHaveCreateLink() => _assert.ShouldNotHaveCreateLink();

        [Test]
        public void ShouldHaveNextLink() => _assert.ShouldHaveNextLink($"/tools/{_language}/words", 4, 10, new KeyValuePair<string, string>("query", _searchedWord));

        [Test]
        public void ShouldHavePreviousLinks() => _assert.ShouldHavePreviousLink($"/tools/{_language}/words", 2, 10, new KeyValuePair<string, string>("query", _searchedWord));
        ...
        public void ShouldReturnExpectedData()
        {
            var expectedItems = CommonWordBuilder.Words
                ...
                .OrderBy(a => a.Word).Skip(2 * 10).Take(10);
